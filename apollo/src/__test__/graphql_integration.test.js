...
import apolloServer from './apolloTestServer.js';
import { getLatestStatus } from '../datasources/firebaseUtils.js';
...
describe('firebase env setup', () => {
  ...
  beforeAll(async () => {
    ...
    testEnv = await initializeTestEnvironment({ projectId: testProjectId });
    /**
     * Warning: Do not use firestore instance from authContext test utility
     * it will cause servertimestamp function failed
     */

    const admin = await mockFirebaseAdmin(testProjectId);

    userInfoAfterAccountCreated = {
      uid: await addTestAccountToAuthEmulator(admin.auth()),
      logIn: true,
    };
  ...
  afterAll(async () => {
    // https://github.com/firebase/quickstart-testing/blob/master/unit-test-security-rules-v9/test/firestore.spec.js#L61
